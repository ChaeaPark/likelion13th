...
const CommonButton = ({ children, onClick, disabled, className = '', variant = 'default', type = 'button' }) => {
  const [clicked, setClicked] = useState(false);

  const base = 'font-semibold rounded-md transition';
  const variants = {
    signin: 'w-full md:w-[120px] py-2 md:py-3 text-sm md:text-base bg-indigo-600 text-white hover:bg-indigo-700',
